...
function solution(N, P, Q) {
   ...
   var primes = new Array(N),
       ...
       //first find primes between 1 and N with sieve of artostenes
       for (i = 2; i <= maxLoop; i++) {
         if (!primes[i]) {
            k = i*i;
            while(k <= N) {
   ...
   //walk and find semiprimes, store the count in an array
   //check for non primes element, and divide it for their smallest divisor
   for (i=0; i<= N;i++) {
       if (primes[i] && !primes[i/primes[i]]) {
           semiCount++;
       }
       semiprimes[i] = semiCount;
   }
   
   //add the results : the amount of semiprimes at the end of the range, minus the number of semiprimes at the start of the array
   for (var j = 0; j < P.length; j++) {
       res = semiprimes[Q[j]] - (semiprimes[P[j]-1] || 0); 
       results.push(res);
    }
